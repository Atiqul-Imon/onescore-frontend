      ...
      <div className="p-4 sm:p-6 space-y-4 sm:space-y-6">
        ...
        {match.series && (
          <div className="pb-3 sm:pb-4 border-b border-gray-200">
            <h3 className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5 sm:mb-2">Series</h3>
            <p className="text-gray-900 font-semibold text-sm sm:text-base break-words">{match.series}</p>
            {match.round && (
        ...
        {match.matchNote && (
          <div className="pb-3 sm:pb-4 border-b border-gray-200">
            <h3 className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5 sm:mb-2">Match Status</h3>
            <p className="text-gray-900 font-semibold text-sm sm:text-base break-words">{match.matchNote}</p>
            {match.target && (
              <p className="text-xs sm:text-sm text-primary-700 font-bold mt-1">Target: {match.target} runs</p>
            )}
          </div>
        )}
        ...
        {/* Format */}
        <div className="pb-3 sm:pb-4 border-b border-gray-200">
          <h3 className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5 sm:mb-2">Format</h3>
          <p className="text-gray-900 font-semibold text-sm sm:text-base">{match.format?.toUpperCase() || 'MATCH'}</p>
        </div>
        ...
        <div className="pb-3 sm:pb-4 border-b border-gray-200">
          ...
          <p className="text-xs sm:text-sm text-gray-600 mt-1 font-medium">{formatTime(match.startTime)}</p>
        </div>
          ...
          <div className="space-y-2 sm:space-y-3">
            <div className="flex items-center gap-2 sm:gap-3 p-2 sm:p-3 rounded-xl bg-primary-50 border border-primary-100">
              ...
              <div className="min-w-0 flex-1">
                <p className="font-bold text-gray-900 text-sm sm:text-base truncate">{match.teams.home.name}</p>
                <p className="text-xs sm:text-sm text-gray-600 font-medium">{match.teams.home.shortName}</p>
              </div>
            </div>
            <div className="flex items-center gap-2 sm:gap-3 p-2 sm:p-3 rounded-xl bg-primary-50 border border-primary-100">
              <span className="text-xl sm:text-2xl flex-shrink-0">{match.teams.away.flag}</span>
              <div className="min-w-0 flex-1">
                <p className="font-bold text-gray-900 text-sm sm:text-base truncate">{match.teams.away.name}</p>
                <p className="text-xs sm:text-sm text-gray-600 font-medium">{match.teams.away.shortName}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        {/* Status */}
        <div>
          <h3 className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-2 sm:mb-3">Status</h3>
          <span
