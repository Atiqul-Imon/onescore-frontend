    ...
    const fetchTeamStats = async () => {
      try {
        ...
        const matchResponse = await fetch(
          `${apiBase}/api/v1/cricket/matches/${matchId}?t=${Date.now()}`
        );
        if (!matchResponse.ok) {
        ...
        <div className="mb-8">
          ...
          <div className="bg-gradient-to-r from-primary-600 to-primary-700 rounded-2xl p-6 text-white">
            <div className="flex items-center gap-4 mb-2">
              ...
              <div>
                ...
                <p className="text-primary-100 text-sm">
                  {match.name || `${match.teams.home.name} vs ${match.teams.away.name}`}
                </p>
              </div>
            </div>
            <div className="mt-4 flex flex-wrap gap-4 text-sm">
              ...
              {match.series && (
                <span className="px-3 py-1 bg-white/20 rounded-full">{match.series}</span>
              )}
          ...
          {batting.length === 0 && bowling.length === 0 && (
            <div className="rounded-lg border border-gray-200 bg-white p-8 text-center">
              <p className="text-gray-600">
                No statistics available for this team in this match yet.
              </p>
            </div>
          )}
        </div>
